...
<!--	<audio-player file='product.file'></audio-player>-->
           ...
           <span class="invisible"> {{qid}} {{e.qid}}</span>
            ...
            <div id="results">
          <span v-if="correctA" class="textGood">Great! This is correct answer</span>
            <span v-if="!correctA" class="textBad">I'm sorry this is wrong answer. Good answer is {{ e.correct}}</span>
          </div>
        </div>
        
    <div class="buttons">
      <button id="ChAItem" class="btn btn-dark" v-on:click="checkA" >Check answer</button>
       <button id="nextItem" class="btn btn-dark" v-on:click="nextQuestion" >Next question</button>
      </div>
...
this.$store.commit("increaseTnumber");
    ...
    resetBA(){
     ...
     Fetchquestions(){
      ...
      db.collection("triadsE").where("qid","==",this.qid).get().then(snapshot=>{
...
this.Fetchquestions();
  ...
  nextTriad(){ console.log("next Triad f"+this.tNumber);
    if(this.tNumber == 1){
      console.log("LAMBADA")
    ...
    nextQuestion(){
      ...
      if(this.checkIfSelected() == true){
        ...
        console.log("next yes");
         //document.getElementById("keyBox").style.display = "flex";
        //document.getElementById("prl1").style.display = "none";
